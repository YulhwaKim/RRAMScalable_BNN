-- This version is based on cuDNN 5.x
-- If you want to use cuDNN version above 5, e.g. cuDNN 7, you have to modify this code

local SpatialConvolution, parent =
    torch.class('cudnn.SpatialConvolutionIntraC', 'nn.SpatialConvolution')
...
function SpatialConvolution:__init(nInputPlane, nOutputPlane,
    ...
    self.nMaxInputPlaneGroup = nMaxInputPlaneGroup or (nInputPlane / self.groups)
    ...
    self.reset = nil
    self.weightMasking = nil
end

-- make the weight mask for the Intra Channel Splitting
function SpatialConvolution:weightMasking(kW, kH)
  local kSpatial = kW * kH
  -- input that each group has to process
  local inputGroup = (self.nInputPlane * kSpatial) / self.groups
  local nOutputPlaneGroup = self.nOutputPlane/self.groups
  local weight_size = self.weight:size()
  self.mask = self.weight.new():resizeAs(self.weight):fill(0)
  -- resize mask of each kernel linear. It makes masking residue block easier.
  self.mask:resize(weight_size[1], weight_size[2]*weight_size[3]*weight_size[4])
  local in_offset = 0
   for i = 0, self.groups-1 do
     local in_residue = kSpatial - in_offset
     local remain = inputGroup - in_residue
     local fullC = math.floor(remain / kSpatial)
     local out_residue = remain % kSpatial
     local startMask = 1+in_offset
     local lengthOne = in_residue + fullC * kSpatial + out_residue
     self.mask[{{i*nOutputPlaneGroup+1, (i+1)*nOutputPlaneGroup},{startMask, startMask+lengthOne-1}}]:fill(1)
     self.nInputPlaneMoveGroup[i+1] = fullC + 1
     in_offset = out_residue
  end
  self.mask:resize(weight_size[1], weight_size[2], weight_size[3], weight_size[4])
  -- masking the weight
...
function SpatialConvolution:checkInputChanged(input)
    ...
    if not self.weightDesc_last then self:resetWeightDescriptors() end
    if not self.weightDesc_last then error "Weights for last layer is not assigned!" end
...
function SpatialConvolution:createIODescriptors(input)
   ...
   if SpatialConvolution.checkInputChanged(self, input) then
        ...
        local input_slice = input:narrow(2,1,self.nMaxInputPlaneGroup)
        self.iDesc = cudnn.toDescriptor(input_slice)

        local input_slice_last = input:narrow(2,1,self.nInputPlaneMoveGroup[self.groups])
        self.iDesc_last = cudnn.toDescriptor(input_slice_last)
        ...
        self.input_offset = iH * iW
        self.output_offset = self.nOutputPlane / self.groups * oH * oW
        self.weight_offset = self.nMaxInputPlaneGroup * self.nOutputPlane / self.groups * kH * kW
...
function SpatialConvolution:updateOutput(input)
    ...
    for g = 0, self.groups - 1 do
      if g < self.groups - 1 then
        ...
        input_offset = input_offset + self.input_offset * self.nInputPlaneMoveGroup[g+1]
      else
        checkedCall(self,'cudnnConvolutionForward', cudnn.getHandle(),
                    cudnn.scalar(input, 1),
                    self.iDesc_last[0], input:data() + input_offset,
                    self.weightDesc_last[0], self.weight:data() + g*self.weight_offset,
                    self.convDesc[0], fwdAlgo,
                    extraBuffer, extraBufferSize,
                    cudnn.scalar(input, 0),
                    self.oDesc[0], self.output:data() + g*self.output_offset);
    ...
    for g = 0,self.groups - 1 do
      if g < self.groups - 1 then
        checkedCall(self,'cudnnConvolutionBackwardData', cudnn.getHandle(),
                    ...
                    extraBuffer, extraBufferSize,
                    cudnn.scalar(input, 0),
                    self.iDesc[0], self.gradInput:data() + input_offset)
        input_offset = input_offset + self.input_offset * self.nInputPlaneMoveGroup[g+1]
      else
        checkedCall(self,'cudnnConvolutionBackwardData', cudnn.getHandle(),
                    cudnn.scalar(input, 1),
                    ...
                    extraBuffer, extraBufferSize,
                    cudnn.scalar(input, 0),
                    self.iDesc_last[0], self.gradInput:data() + input_offset) 
      end
    end
...
function SpatialConvolution:accGradParameters(input, gradOutput, scale)
    ...
    for g = 0, self.groups - 1 do
      ...
      checkedCall(self,'cudnnConvolutionBackwardFilter', cudnn.getHandle(),
                   ...
                   extraBuffer, extraBufferSize,
                   cudnn.scalar(input, 1),
                   self.weightDesc_last[0], self.gradWeight:data() + g*self.weight_offset);
     end
    end
